...
rows3.push(subtotalRow('給与計算代行　小計', '月額／年額', '14,400', '172,800'));

rows3.push(totalRow('小　計（税抜）　　', '172,800'));
rows3.push(totalRow('消費税（10％）　　', '17,280'));
rows3.push(totalRow('お値引き（税込）　※3　　', '△50,160'));
...
// ※3 はお値引き欄から参照するため、共通注記より前に差し込む
NOTES3.splice(2, 0, 'お値引きは、従前の顧問契約書（令和6年1月改定。給与計算代行 月額 金11,660円（税込））に基づく年額報酬 金139,920円（税込）に本別紙の合計額を据え置くための調整である。お値引き額は税抜 金45,600円（消費税額 金4,560円）に相当する。適用期間は別紙4第11条による。');
