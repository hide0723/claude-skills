...
rows1.push(itemRow('', 'なお、年仕訳数1,201件～＠50にて別途請求', null, '4,325　件', '50', '216,250',
  'R8.4月期　年間仕訳数 5,525件'));
...
rows1.push(subtotalRow('税務業務　小計', '月額／年額', '', ['月額　￥65,000', '年額　￥996,250']));
...
rows1.push(subtotalRow('決算・申告代行業務　小計', '年額のみ', '－', ['年額　￥200,000']));

rows1.push(totalRow('小　計（税抜）　　', '1,196,250'));
rows1.push(totalRow('消費税（10％）　　', '119,625'));
rows1.push(totalRow('お値引き（税込）　※3　　', '△682,275'));
...
rows3.push(itemRow('3', '給与計算　勤怠の集計有：人数×月＠400×12ヶ月（年＠4,800）※1', true, '13　人', '4,800', '62,400'));
rows3.push(itemRow('4', '給与明細印刷有：人数×月＠200×12ヶ月（年＠2,400）', true, '13　人', '2,400', '31,200'));
rows3.push(itemRow('5', '納税代行手続き（ダイレクト納付）－住民税＠1,000', true, '12　ヶ月', '1,000', '12,000'));
rows3.push(itemRow('6', '労働保険・社会保険手続き一式：＠4,000', true, '12　ヶ月', '4,000', '48,000', '※2'));
rows3.push(itemRow('7', '労働保険の算定基礎届の作成：＠45,000', true, '1　回', '45,000', '45,000'));

rows3.push(subtotalRow('給与計算代行　小計', '月額／年額', '', ['月額　￥27,200', '年額　￥371,400']));

rows3.push(totalRow('小　計（税抜）　　', '371,400'));
rows3.push(totalRow('消費税（10％）　　', '37,140'));
rows3.push(totalRow('お値引き（税込）　※3　　', '△268,620'));
...
NOTES1.splice(2, 0, 'お値引きは、従前の顧問契約書（令和6年1月改定。税務顧問報酬 月額 金52,800円（税込））に基づく年額報酬 金633,600円（税込）に本別紙の合計額を据え置くための調整である。お値引き額は税抜 金620,250円（消費税額 金62,025円）に相当する。適用期間は別紙4第11条による。');

const NOTES3 = [
  '出勤簿及びタイムカードの預かり（確認業務含む）の場合は、勤怠の管理有となります。',
  '労働・社会保険の手続代行の範囲は、別紙4第10条に定めるところによる。甲が別途社会保険労務士に委託している業務は含まない。',
  '「含む」欄に☑を付していない業務は、本契約に含まれない。甲が当該業務を希望する場合は、その都度、事前に甲乙が業務内容及び報酬額を合意のうえ実施し、上表の単価により別途請求する。',
  '上表に掲げのない業務のうち、年末調整代行、支払調書の作成代行、償却資産税申告書の作成代行その他の業務の報酬は、別紙2のとおりとする。',
  '本別紙は本契約の一部を構成する。本別紙の変更は、甲乙双方の書面又は電磁的方法による合意によらなければ、その効力を生じない。',
  '契約期間の中途において消費税率が改正された場合、消費税額は改正後の税率による。',
...
// ※3 はお値引き欄から参照するため、共通注記より前に差し込む
NOTES3.splice(2, 0, 'お値引きは、従前の顧問契約書（令和6年1月改定。給与計算代行 月額 金11,660円（税込））に基づく年額報酬 金139,920円（税込）に本別紙の合計額を据え置くための調整である。お値引き額は税抜 金244,200円（消費税額 金24,420円）に相当する。適用期間は別紙4第11条による。');
...
const besshi4 = [
  ...
  art('第10条（労働・社会保険の手続代行の範囲）'),
  cl('1　本契約第6項に定める「必要な労働・社会保険の手続代行」とは、次に掲げる手続をいう。'),
  cl('　　（　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　）'),
  cl('2　甲が別途社会保険労務士に委託している次の業務は、前項に含まない。'),
  cl('　　（　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　）'),
  cl('3　前2項の記載は、甲乙協議のうえ確定する。'),
