// 税理士法人 福田会計 顧問契約書 別紙　株式会社岡田ＨＤ版
// 共通フォーム（別紙1〜3）に見積書 20260725-004 の数値を記入し、但し書きを別紙4にまとめたもの。
const {
...
function frontMatter(title, subtitle, lead) {
  return [
    ...
    p('原契約　　　　令和8年2月　　日付　顧問契約書', { size: 18 }),
    p('対象期間　　　令和8年3月1日から令和9年2月28日まで', { size: 18 }),
    blank(),
    p(lead, { size: 17 }),
    blank(80),
  ];
...
rows1.push(itemRow('', 'なお、1事業年度1,200仕訳を超える部分は＠50にて別途請求', null, '', '', '', 'R8.2月期 総仕訳数 2,541件／決算時に精算'));
...
rows1.push(itemRow('5', '事業規模：', true, '12　ヶ月', '5,000', '60,000', '7,412万円想定　※3',
  ['年商 7,412 万円（年商5,000万円未満は＠0円、5,000万円超から5,000万円ごとに＠5,000）',
   '※5億円超は一律（+50,000円）']));

rows1.push(itemRow('6', '報酬支払：口座振替以外＠3,000　※請求書発行を行う場合',
  false, '―　ケ月', '3,000', '0', '口座振替'));

rows1.push(subtotalRow('税務業務　小計', '月額／年額', '25,000', '300,000'));

rows1.push(itemRow('7', '決算報酬：法人 ＠120,000　個人＆NPO ＠50,000', true, '1　事業年度', '120,000', '120,000', '法人'));
rows1.push(itemRow('8', '消費税申告：＠50,000', false, '―　事業年度', '50,000', '0', '※5'));
...
rows1.push(itemRow('12', 'NXPRO使用　決算手数料：年間△50,000（ミロクのクラウドアプリを使用している場合）',
  false, '―　事業年度', '△50,000', '0'));
rows1.push(itemRow('13', '税理士法第33条の2第1項に規定する書面添付', true, '1　事業年度', '30,000', '30,000', '※4'));

rows1.push(subtotalRow('決算・申告代行業務　小計', '年額のみ', '－', '150,000'));

rows1.push(totalRow('小　計（税抜）　　', '450,000'));
rows1.push(totalRow('消費税（10％）　　', '45,000'));
rows1.push(totalRow('お値引き（税込）　※6　　', '△158,400'));
rows1.push(totalRow('合　計（税込）　　', '336,600', { bold: true, shade: 'E6E6E6' }));
...
rows3.push(subtotalRow('給与計算代行　小計', '月額／年額', '4,800', '102,600'));

rows3.push(totalRow('小　計（税抜）　　', '102,600'));
rows3.push(totalRow('消費税（10％）　　', '10,260'));
rows3.push(totalRow('お値引き（税込）　※3　　', '－'));
rows3.push(totalRow('合　計（税込）　　', '112,860', { bold: true, shade: 'E6E6E6' }));
...
const NOTES1 = [
  ...
  '項目5の年商は、甲を受託者又は貸主とする次の契約に基づき、令和8年2月期末時点の条件が12か月継続するものとして年間換算した年額（税抜）である。令和8年2月期は期の中途で不動産賃貸の条件が変動しているため、同期の実額ではなく年間換算額により事業規模を判定している。①業務委託（ITライセンス・運用保守及び経営管理支援）月額5,500,000円（税込）＝税抜5,000,000円、②什器備品等賃貸借 月額770,000円（税込）＝税抜700,000円、③不動産賃貸借3件（土地〔与六分136番地1〕70,000円、建物・駐車場〔与六分136番地3〕第2期間 屋内駐車場4台120,000円・屋外駐車場3台30,000円・建物210,000円（税抜）、社宅〔与六分131番地11〕60,000円）＝税抜476,364円。月額合計 税抜6,176,364円×12か月＝74,116,368円。5,000万円以上1億円未満の区分により月額＠5,000を加算する。',
  '税理士法第33条の2第1項に規定する書面添付は、見積書に金額の記載がないが、従前の顧問契約書において委任範囲に含まれていたことから、引き続き実施するものとして計上している。',
  '甲は令和10年2月期より消費税の課税事業者となる予定である。本契約の対象期間（令和9年2月期）は免税事業者であるため、項目8の消費税申告は計上していない。令和10年2月期以降の契約においては、同項目を計上するものとする。',
  'お値引きは、従前の顧問契約書（令和6年5月29日付。税務顧問報酬 月額 金28,050円（税込）。決算報酬及び書面添付を含む。）に基づく年額報酬 金336,600円（税込）に本別紙の合計額を据え置くための調整である。お値引き額は税抜 金144,000円（消費税額 金14,400円）に相当する。適用期間は別紙4第11条による。',
...
const NOTES3 = [
  ...
  '労働保険・社会保険の手続代行（項目6）は、本契約に含まない。甲が別途委託する社会保険労務士が行う。労働保険の算定基礎届の作成（項目7）は本契約に含む。別紙4第10条を参照。',
  '給与計算代行業務は、従前の顧問契約書の委任範囲に含まれていない新規の受任業務であるため、お値引きの対象としていない。',
  '「含む」欄に☑を付していない業務は、本契約に含まれない。甲が当該業務を希望する場合は、その都度、事前に甲乙が業務内容及び報酬額を合意のうえ実施し、上表の単価により別途請求する。',
  '上表に掲げのない業務のうち、年末調整代行、支払調書の作成代行、償却資産税申告書の作成代行その他の業務の報酬は、別紙2のとおりとする。',
  '本別紙は本契約の一部を構成する。本別紙の変更は、甲乙双方の書面又は電磁的方法による合意によらなければ、その効力を生じない。',
  '契約期間の中途において消費税率が改正された場合、消費税額は改正後の税率による。',
...
const RATE_NOTES = [
  ...
  '記帳代行の仕訳数超過分の報酬は、当該事業年度の仕訳数が確定した後、決算時に一括して請求する。このため、別紙1には金額を記載しない。詳細は別紙4第2条に定めるところによる。',
...
const besshi4 = [
  art('第1条（税額控除に係る報酬）'),
  cl('1　別紙2の区分7に定める税額控除に係る報酬の上限￥500,000（税抜）は、1事業年度あたりの上限とする。'),
  ...
  cl('2　別紙1の項目2に係る記帳代行報酬に含まれる仕訳数は、1事業年度あたり1,200仕訳とする。これを超える部分の単価は、1仕訳あたり￥50（税抜）とする。当該報酬は、当該事業年度の仕訳数が確定した後、決算時に一括して請求する。このため、別紙1には金額を記載しない。'),
  cl('3　前項の仕訳数は、会計ソフトに記録された仕訳の件数により算定する。ただし、次に掲げる仕訳は算入しない。'),
  cl('　一　取消、赤伝、再生成その他システム上の理由により重複して記録された仕訳'),
  cl('　二　会計ソフトの仕様により自動的に生成される決算振替仕訳その他、乙の判断を要しない仕訳'),
  cl('4　乙は、各事業年度の仕訳数が確定したときは、発生元別（乙による手入力、外部データ連携、決算整理その他）の件数を記載した明細を甲に提示する。甲乙間で仕訳数について疑義が生じたときは、当該明細に基づき協議のうえ確定する。'),
  ...
  cl('1　本契約の期間は、令和8年3月1日から令和9年2月28日までとする。'),
  ...
  cl('1　労働保険及び社会保険の手続代行（別紙3の項目6）は、本契約に含まない。当該業務は乙の受任範囲外とし、甲が別途委託する社会保険労務士が行う。'),
  cl('2　前項にかかわらず、労働保険の算定基礎届の作成（別紙3の項目7）は、本契約に含む。'),
  cl('3　本契約に「必要な労働・社会保険の手続代行を含む」旨の記載がある場合は、前2項により読み替えるものとする。'),
  cl('4　甲が将来第1項の業務を乙に委任することを希望する場合は、別紙3の項目6の単価により、その都度、事前に甲乙が合意のうえ実施する。'),

  art('第11条（お値引き）'),
  cl('1　別紙1のお値引きは、従前の顧問契約書（令和6年5月29日付）に基づく年額報酬 金336,600円（税込。税務顧問報酬 月額 金28,050円（税込）。決算報酬及び書面添付を含む。）に、別紙1の合計額を据え置くための調整である。'),
  cl('2　別紙3の給与計算代行業務は、従前の顧問契約書の委任範囲に含まれていない新規の受任業務であるため、お値引きの対象としない。'),
  cl('3　第1項のお値引きは、本契約の期間（令和8年3月1日から令和9年2月28日まで）に限り適用する。次期以降の取扱いは、甲乙協議のうえ決定する。'),
  cl('4　本契約期間の中途において委任業務の範囲又は数量に変更が生じた場合のお値引き額の取扱いは、甲乙協議のうえ決定する。'),
];
