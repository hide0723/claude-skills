...
rows1.push(itemRow('14', '税理士法第33条の2第1項に規定する書面添付', false, '―　事業年度', '30,000', '0', '※5'));

rows1.push(subtotalRow('決算・申告代行業務　小計', '年額のみ', '－', '120,000'));

rows1.push(totalRow('小　計（税抜）　　', '405,000'));
rows1.push(totalRow('消費税（10％）　　', '40,500'));
rows1.push(totalRow('お値引き（税込）　※7　　', '△108,900'));
...
const NOTES1 = [
  ...
  '税理士法第33条の2第1項に規定する書面添付は、見積書に金額の記載がないため本表では契約外としている。従前の顧問契約書では委任範囲に含まれていたため、要確認事項である。',
  '甲は令和10年2月期より消費税の課税事業者となる予定である。本契約の対象期間（令和9年2月期）は免税事業者であるため、項目8の消費税申告は計上していない。令和10年2月期以降の契約においては、同項目を計上するものとする。',
  'お値引きは、従前の顧問契約書（令和6年5月29日付。税務顧問報酬 月額 金28,050円（税込）。決算報酬及び書面添付を含む。）に基づく年額報酬 金336,600円（税込）に本別紙の合計額を据え置くための調整である。お値引き額は税抜 金99,000円（消費税額 金9,900円）に相当する。適用期間は別紙4第11条による。',
